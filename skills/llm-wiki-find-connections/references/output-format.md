...
- **Spreading activation**: graph traversal over note edges (when using `--mode spreading`)
- **Typed-edge enrichment**: edge types (derives-from, instantiates, references, associates, tension) via `resources/brain-graph/run_brain_graph.sh inspect "Note" --json`
