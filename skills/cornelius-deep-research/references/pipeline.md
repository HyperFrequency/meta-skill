...
1. Read KB analysis: `cat knowledge-base-analysis.md`
2. Check recent activity: `ls -lt Brain/Document\ Insights/ | head -10`
...
4. Select 1-3 topics that fill gaps, build on existing strengths (e.g.
   Buddhism-Neuroscience-AI triangle), connect underexplored domains, add
   empirical validation to intuitive frameworks, or challenge current thinking.

**Good topic examples:** "Neuroscience of habits and behavior change";
"Collective intelligence and swarm behavior"; "Embodied cognition and
interoception"; "Complexity science and emergence"; "Creativity neuroscience and
insight generation" — chosen when that domain is underrepresented.
...
Create session folder `Brain/Document Insights/[YYYY-MM-DD Topic Description]/`.
...
2. Bridge to existing KB: connect to the 6 primary hubs (Consciousness, Dopamine,
   Decision-Making, Identity, AI Agents, Flow States); reference existing
   frameworks (Folder Paradigm, Mental Models Taxonomy); identify consilience
   (3+ domains converging); find validation/challenges to current thinking.
...
Brain/Document Insights/[Session-Folder]/. Write a comprehensive changelog.
```
...
> Responses save to `Brain/AI Extracted Notes/` and the connection finder maps
> both sets together. Say **yes** to run, or **skip** to go straight to
> connection discovery."

- **If yes:** invoke the `insight-interview` skill for the current topic. Dialogue
  runs one question at a time; user insights save to `Brain/AI Extracted Notes/`;
  note the session timestamp so Phase 5 includes these notes.
- **If skip:** proceed to Phase 5.

If the interview ran, Phase 5 maps connections across BOTH the external research
session folder and the new personal notes in `Brain/AI Extracted Notes/`.
...
STARTING POINTS: all notes in Brain/Document Insights/[Session-Folder]/
(or specify individual notes for targeted passes).

GOALS:
1. Bridge to existing knowledge: connect to existing AI insights; link to the 6
   primary hubs; relate to original frameworks (Folder Paradigm, Mental Models
   Taxonomy); map to MOCs and output content.
2. Cross-domain opportunities: Buddhism↔Neuroscience↔AI consilience;
   Decision Science↔Agent Architecture; Flow↔Peak Performance↔AI Optimization;
   Identity/Belief↔Agent Fitness Functions; Dopamine hub (universal bridge).
...
5. Output: map direct connections; identify bridge notes; highlight consilience
   zones; changelog `CHANGELOG - Connection Discovery Session YYYY-MM-DD.md` in
   Brain/05-Meta/Changelogs/; update Brain/CHANGELOG.md; suggest concrete article
   topics or framework extensions.
```

**Monitor:** verify changelog in `Brain/05-Meta/Changelogs/`, master
`CHANGELOG.md` updated, note consilience zones / synthesis opportunities, identify
