...
```bash
VENV_PATH=$HOME/Desktop/alpha_gen/.venv-vectorbtpro-sys/bin/activate \
RESULTS_ROOT=$HOME/Desktop/backtests_new \
STATE_ROOT=$HOME/Desktop/backtests_new/state \
LOGS_ROOT=$HOME/Desktop/backtests_new/logs \
bash scripts/start_swarm_watchdogs.sh
```
...
# 3. Generate a performance tearsheet (quantstats-rs) + MAE/leverage from a trades CSV
python neuro-quant/tearsheet-generator/scripts/generate_tearsheet.py \
  --trades /path/to/trades.csv --capital 10000 --mae \
  --strategy-title SOL_MTF_EMA_001 -o ./tearsheets/SOL_MTF_EMA_001.html
```
...
- Agent registry roles loaded: `30` from `$HOME/Desktop/alpha_gen/backtest_optimize/agents/AGENT_REGISTRY.md`
- agents.db tables: `27` at `$HOME/.agentdb/vectors.db` (skills=0, events=0)
