...
description: >
  End-to-end strategy development ORCHESTRATION runbook: ideation -> signal generation ->
  prescreen/backtest -> parameter optimization -> walk-forward validation -> tearsheet, with an
  autonomous swarm doctrine (watchdogs, fill lanes, GPU/CPU orchestration, Vast.ai deploy) and a
  worked MTF-EMA/Heikin-Ashi/VW-Bollinger reference strategy. Use when the user wants the WHOLE
  pipeline from idea to validated strategy. Trigger on "build a strategy end to end", "run the
  full backtest and optimize workflow", "take this idea to a validated strategy". This is the
  orchestrator and delegates each step: for vectorized backtests use vectorbt; for the
  event-driven engine use nautilus-trader; for WFO epoch selection use adaptive-wfo-epoch; for
  distributed Optuna/Ray sweeps use neuro-quant-distributed-optimization; to verify vs an Optuna
  baseline use strategy-verify; for the tearsheet use tearsheet-generator.
version: "2.0.0"
...
Generate performance tear sheets (owned by the `tearsheet-generator` skill, quantstats-rs engine):

```bash
# From a returns series (date,return CSV):
quantstats-rs report --returns returns.csv -o ./tearsheets/STRATEGY_NAME.html
# From a trades CSV (derives returns, adds the MAE/leverage JSON):
python neuro-quant/tearsheet-generator/scripts/generate_tearsheet.py \
  --trades /path/to/trades.csv --capital 10000 --mae \
  --strategy-title STRATEGY_NAME -o ./tearsheets/STRATEGY_NAME.html
```

See the `tearsheet-generator` skill for MAE analysis, leverage recommendations, and layout options.
...
- [quantstats-rs](https://github.com/HyperFrequency/quantstats-rs-private) — Rust QuantStats engine (Python reference: ranaroussi/quantstats)
