...
The structure above is formalized as a JSON Schema in [output-schema.json](output-schema.json). When deterministic machine-readable output is required, validate the critique against it (for example `ajv validate -s output-schema.json -d critique.json`) and list issues in descending severity order (critical first).
