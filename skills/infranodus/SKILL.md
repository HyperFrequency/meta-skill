...
version: 0.1.0
description: >
  Knowledge graph + text network analysis via the LOCAL InfraNodus MCP (http://infranodus-
  mcp:9005/sse). Use when the user types /infranodus <tool> [args], wants to generate a knowledge
  graph from text, find topical clusters, identify content gaps, propose research questions, or
  analyze text structure/bias/coherence. Trigger on "build an ontology from these notes", "what
  are the topical clusters in X", "find content gaps", "generate research questions from Y",
  "analyze the bias in this text", or any text-network/knowledge-graph framing. Runs against the
  local OSS engine — no infranodus.com calls, no per-request fees. Do NOT use for the mcporter CLI
  wrapper (use infranodus-cli).
---

# infranodus

Knowledge graph + text network analysis via the LOCAL InfraNodus MCP (`http://infranodus-mcp:9005/sse`). Use when the user types `/infranodus <tool> [args]`, wants to generate a knowledge graph from text, find topical clusters, identify content gaps, propose research questions, or analyze text structure / bias / coherence. Trigger on phrases like "build an ontology from these notes", "what are the topical clusters in X", "find content gaps", "generate research questions from Y", "analyze the bias in this text", or any text-network / knowledge-graph framing. Important: runs against the local OSS engine — no calls to infranodus.com, no per-request fees. AI-dependent tools (research-ideas) degrade gracefully because the OSS engine is older than the SaaS.
