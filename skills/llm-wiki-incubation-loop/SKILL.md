---
name: llm-wiki-incubation-loop
version: 0.2.0
description: Autonomous multi-cycle thinking loop over your Obsidian/turbovault knowledge base - processes active thinking-registry topics one rotating analytical move at a time (ACH, Bayesian updating, steelmanning, cross-domain bridging, implication/assumption audits) and persists reasoning state across scheduled runs until convergence. Use WHEN an open question needs slow refinement whose depth accumulates across many daily runs with no human in each loop, or to seed a new thinking topic / check convergence status. Do NOT use for one-off fact lookups, time-critical decisions with a deadline, single-pass reasoning on a question (use llm-wiki-dialectic or llm-wiki-deep-research), profiling the vault's overall structure/clusters (llm-wiki-analyze-kb), folding new notes into the KB (llm-wiki-integrate-recent-notes), or crystallizing a converged topic into a permanent note (the manual llm-wiki-synthesize-insights step).
...
- Add note: `**CONVERGED** - Ready for crystallization via llm-wiki-synthesize-insights`
...
2. Run the `llm-wiki-synthesize-insights` skill on `[topic-slug]` to graduate conclusions to a permanent note
...
- [ ] **Version control**:
  - [ ] `git add neuro-base/llm-wiki-incubation-loop/SKILL.md`
