...
name: ml-pipeline
description: >
  End-to-end machine-learning pipeline for trading: feature engineering, AutoML, deep learning,
  and financial RL, with bundled runnable scripts (data validation, feature engineering, feature
  importance, model evaluation, feature store, deployment) and an anti-leakage discipline
  (purged/embargoed CV, in-fold target encoding). Use to build or run an ML training pipeline for
  a trading model. Trigger on "build an ML pipeline", "automated feature engineering", "AutoML
  sweep", "train a model on these features", "anti-leakage validation". For the forecasting model
  zoo (TabPFN/chronos/HMM/DeepLOB) use neuro-quant-forecasting-oracle-ml; for model VALIDATION
  discipline (PBO, purged CV, deflated Sharpe) use model-evaluation; for standalone feature
  pipelines use feature-engineering; for distributed Optuna/Ray sweeps use
  neuro-quant-distributed-optimization.
version: "2.0.0"
