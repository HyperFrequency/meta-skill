# CPU → GPU Code Transformation Patterns

Before/after templates for converting common CPU code to GPU. Read the matching per-library
reference (e.g. `cupy.md`, `cudf.md`) for the full API before writing real code.
...
## Static plots → GPU dashboard with cuxfilter
```python
