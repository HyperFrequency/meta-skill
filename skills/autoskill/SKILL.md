...
name: autoskill
version: 0.1.0
description: Observe the user's screen via screenpipe, detect repeated research workflows, match them against existing scientific-agent-skills, and draft new skills (or composition recipes that chain existing ones) for the patterns not yet covered. Use when the user asks to analyze their recent work and propose skills based on what they actually do. Requires the screenpipe daemon (https://github.com/screenpipe/screenpipe) running locally on port 3030 — the skill has no other data source and will refuse to run if screenpipe is unreachable. All detection runs locally; only redacted cluster summaries reach the LLM. Not for one-off screenpipe API questions, real-time screen monitoring, or automatic invocation — must be user-triggered.
...
Four dependencies: the **screenpipe daemon** (on `localhost:3030`), a **screenpipe API token** (`export SCREENPIPE_TOKEN=$(screenpipe auth token)`), a **Python env** (`pipenv install httpx pyyaml sentence-transformers`), and an **LLM backend** (local LM Studio by default; `claude`/`foundry` opt-in). Full install steps — building screenpipe from source, macOS permissions, model loading, cloud backend config — are in **`references/setup.md`**. Verify all four at once with `python scripts/autoskill.py doctor`.
